...
z_result find_z(MatrixXd newA, VectorXd b, int x_dim){
    int n = newA.rows();
    int d = newA.cols();
...
fr_result entire_facial_reduction_step(MatrixXd A, VectorXd b, int x_dim){
    z_result z_ans = find_z(A, b, x_dim);
    if(!z_ans.found_sol){
        fr_result f_ans;
        f_ans.A = A;
        f_ans.b = b;
...
fr_result reduce_sampling(MatrixXd M, VectorXd b, int delta_dim){
...
problem_result reduce_problem(MatrixXd A, VectorXd b){
    ...
    MatrixXd M = make_full_rank(res.A);
    fr_result reduced = reduce_sampling(M, res.b, res.A.cols() - x_dim);
